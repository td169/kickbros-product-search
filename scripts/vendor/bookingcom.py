"""
VENDORED FILE — not original work of this project.

Source: https://github.com/scrapfly/scrapfly-scrapers/tree/main/bookingcom-scraper
Commit: 94c8fd6489cf (main, as of 2026-08-18)
Author: Bernardas Alisauskas <bernardas@scrapfly.io> and scrapfly-scrapers contributors
License: Non-Profit Open Software License 3.0 (NPOSL-3.0) — full text:
         https://opensource.org/license/nposl-3-0/

Used here as-is (only this attribution header added) to fetch Booking.com hotel availability
pricing for scripts/scan_trips.py, per the pinned-hotel list in that script. Kept unmodified so
it stays easy to diff against upstream if Booking.com changes their site and the scraper needs
an update from there.

Note on NPOSL-3.0 Section 17(d): that clause concerns further *redistribution* of this file by
someone who isn't a non-profit deriving no revenue from the distribution — it does not restrict
using the code. This repo isn't redistributing this file as a standalone release; it's used
internally as part of a personal trip-planning tool. If that ever changes, revisit under the
parent OSL 3.0 terms per the license text.
"""
...
BASE_CONFIG = {
    ...
    "asp": True,
    "country": "GB",  # changed from the upstream "US" default — Temi's in the UK
}
